...
class _FirebasePolylineMapState extends State<FirebasePolylineMap> {
  ...
  static const _minCameraZoom = 4.0;
        ...
        setState(() {
          ...
          if (mounted) {
            setState(() {
              ...
              // Replace existing markers with custom icons
        ...
        if (data.containsKey('location')) {
          ...
          if (mounted) {
            setState(() {
              ...
              if (mounted) {
                setState(() {
                  // Always show marker immediately (red if custom icon not loaded yet)
                  _markers.removeWhere((m) => m.markerId.value == 'driver');
                  _markers.add(
                    gmaps.Marker(
                      markerId: const gmaps.MarkerId('driver'),
                      position: _driverPosition!,
                      icon: _driverIcon ?? gmaps.BitmapDescriptor.defaultMarker,
                      zIndex: 2,
                    ),
                  );
