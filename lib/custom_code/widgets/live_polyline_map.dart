...
class _LivePolylineMap extends State<LivePolylineMap> {
  ...
  Timer? _destinationRetryTimer;
  bool _destinationAttempted = false;
  ...
  void _setupDestinationStream() {
    if (widget.destinationRef == null) {
      print("Destination reference is null!");
      return;
    }

    // Cancel any existing stream
    _destinationStream?.cancel();
    _destinationAttempted = true;

    _destinationStream = widget.destinationRef!.snapshots().listen((snapshot) {
      print("Received destination snapshot: ${snapshot.data()}");
      ...
      final geoPoint = snapshot.get('location') as GeoPoint?;
      if (geoPoint == null) {
        print("GeoPoint is null in snapshot!");
        return;
      }

      _updateDestination(geoPoint);
      _tryBuildRoute(); // Always try to build route when destination updates
    }, onError: (e) {
      print("Destination stream error: $e");
      // Reset attempt flag to allow retries
      _destinationAttempted = false;
    });
  }

  void _startDestinationRetryTimer() {
    // Retry destination fetch every 2 seconds until we get it
    _destinationRetryTimer =
        Timer.periodic(const Duration(seconds: 2), (timer) {
      if (_currentDestination == null && !_destinationAttempted) {
        print("Retrying destination fetch...");
        _setupDestinationStream();
      } else {
        timer.cancel();
      }
  ...
  void _updateDestination(GeoPoint geoPoint) {
    ...
    if (_currentDestination == null) {
      ...
      if (distanceMoved > _destinationMovementThreshold) {
        print("DESTINATION MOVED: ${geoPoint.latitude}, ${geoPoint.longitude}");
        _currentDestination = newDestination;
        if (mounted) setState(() {});
      }
    }

    // Always try to build route when we have new destination data
    _tryBuildRoute();
  }
  ...
  void _tryBuildRoute() {
    if (_lastPosition == null) {
      print("Cannot build route - position is null");
      return;
    }

    if (_currentDestination == null) {
      print("Cannot build route - destination is null");
      return;
    }

    if (_routeReady) {
      print("Route already built");
      return;
    }

    print("BUILDING ROUTE NOW...");
    _routeReady = true;
  ...
  Widget build(BuildContext context) {
    // Show map if we have EITHER position or destination
